...
def to_string(ll: Layout, user: CminiData):
    author = authors.get_name(ll.user)

    monogram = corpora.ngrams(1, user)
    trigram = corpora.ngrams(3, user)
    ...
    with AbsPath(__file__).open('../likes.json', 'r') as f:
        likes = json.load(f)
