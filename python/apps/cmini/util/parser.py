...
def get_arg(user: CminiData) -> str:
    args = user.message.split()
    return ' '.join(args[2:])


def get_args(user: CminiData) -> list[str]:
    args = user.message.split()
    return args[2:]


def get_layout(user: CminiData) -> tuple[str, str]:
    tokens = user.message.split('```')
    command = tokens[0]
...
def get_kwargs(message: CminiData,
